...
/*    function file() {
    ...
    /**
     * 上传文件
     * @param  [type]     $attachment [$_FILES]
     * @param  [type]     $target     [文件完整路径包含文件名]
     * @param  integer    $maxsize    [大小]
     * @param  integer    $is_image   [是否图片]
    ...
    function uploadfile($attachment,$target,$maxsize=1024,$is_image=1){
        ...
        if($is_image){
            ...
            /* 判断图片后缀 */
            if ( !in_array(file::extname($attach['name']), array('jpg','jpeg','bmp','gif','png')) ) {
                $result['msg'] ='格式错误';
                return $result;
            }
            if($filesize>$maxsize){
