<?php
defined('IN_B2B2C') or exit('Access Invalid!');
/**
 * 金币功能公用
 */
$lang['admin_points_unavailable']	 		= '系统未开启金币功能';
$lang['admin_points_mod_tip']				= '修改金币';
$lang['admin_points_system_desc']			= '管理员手动操作金币';
$lang['admin_points_userrecord_error']		= '会员信息错误';
$lang['admin_points_membername']			= '会员名称';
$lang['admin_points_operatetype']			= '增减类型';
$lang['admin_points_operatetype_add']		= '增加';
$lang['admin_points_operatetype_reduce']	= '减少';
$lang['admin_points_pointsnum']				= '金币值';
$lang['admin_points_pointsdesc']			= '描述';
$lang['admin_points_pointsdesc_notice']		= '描述信息将显示在金币明细相关页，会员和管理员都可见';

/**
 * 金币添加
 */
$lang['admin_points_member_error_again']	= '会员信息错误，请重新填写会员名';
$lang['admin_points_points_null_error']		= '请添加金币值';
$lang['admin_points_points_min_error']		= '金币值必须大于0';
$lang['admin_points_points_short_error']	= '金币不足，会员当前金币数为';
$lang['admin_points_addmembername_error']	= '请输入会员名';
$lang['admin_points_member_tip_1']			= '会员';
$lang['admin_points_member_tip_2']			= ', 当前金币数为';
/**
 * 金币日志
 */
$lang['admin_points_log_title']			= '金币明细';
$lang['admin_points_adminname']				= '管理员名称';
$lang['admin_points_stage']					= '操作阶段';
$lang['admin_points_stage_regist']				= '注册';
$lang['admin_points_stage_login']				= '登录';
$lang['admin_points_stage_login']				= '登录';
$lang['pointsregist_yyy']						='摇一摇';
$lang['admin_points_stage_comments']				= '商品评论';
$lang['admin_points_stage_order']				= '订单消费';
$lang['admin_points_stage_system']				= '金币管理';
$lang['admin_points_stage_pointorder']		= '礼品兑换';
$lang['admin_points_stage_app']		= '金币兑换';
$lang['admin_points_addtime']				= '添加时间';
$lang['admin_points_addtime_to']				= '至';
$lang['admin_points_log_help1']				= '金币明细，展示了被操作人员（会员）、操作人员（管理员）、操作金币数（金币值，“-”表示减少，无符号表示增加）、操作时间（添加时间）等信息';



