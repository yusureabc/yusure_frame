...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?act=merchant&op=storeclass"><span><?php echo $lang['nc_manage'];?></span></a></li>
  ...
  <form id="add_form" method="post" enctype="multipart/form-data" action="index.php?act=merchant&op=<?php echo $output['op'];?>">
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr class="noborder">
          ...
          <script type="text/javascript" SRC="<?php echo RESOURCE_SITE_URL;?>/js/jscolor/jscolor.js"></script>
		<tr class="noborder">
          <td colspan="2" class="required"><label class="validation" for="class_color">颜色</label></td>
        </tr>
		<tr class="noborder">
          <td class="vatop rowform">
          <input type="text" id="cp1" class="color" name="class_color" value="<?php if(isset($output['class_info']['class_color'])) echo $output['class_info']['class_color'];?>"/>
          </td>
        </tr>
          ...
          <td class="vatop rowform">
           <span class="type-file-box">
            ...
            <?php if(!empty($output['class_info']['class_image'])) { ?>
              <img src="<?php echo UPLOAD_SITE_URL.'/o2o/merchant_class/'.$output['class_info']['class_image'];?>">
              <?php } ?>
            </td>
          <td class="vatop tips">建议上传图标尺寸为：78px*78px</td>
        </tr>
        
        <tr>
          <td colspan="2" class="required"><label for="class_s_image">分类小图</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
           <span class="type-file-box">
            <input name="old_class_s_image" type="hidden" value="<?php echo $output['class_info']['class_s_image'];?>" />
            <input name="class_s_image" type="file" class="type-file-file" id="class_s_image" size="30" hidefocus="true" nc_type="microshop_goods_class_s_image">
            </span><br><br>
            <?php if(!empty($output['class_info']['class_s_image'])) { ?>
              <img src="<?php echo UPLOAD_SITE_URL.'/o2o/merchant_class/'.$output['class_info']['class_s_image'];?>">
              <?php } ?>
            </td>
          <td class="vatop tips">建议上传图标尺寸为：30px*30px</td>
        </tr>
...
$(document).ready(function(){
    ...
    var textButton1="<input type='text' name='textfield1' id='textfield2' class='type-file-text' /><input type='button' name='button' id='button1' value='' class='type-file-button' />";
    $(textButton1).insertBefore("#class_s_image");
    
    $("#class_image").change(function(){
        $("#textfield1").val($("#class_image").val());
    });
    
    $("#class_s_image").change(function(){
        $("#textfield2").val($("#class_s_image").val());
    });
	...
	$("input[nc_type='microshop_goods_class_s_image']").live("change", function(){
		var src = getFullPath($(this)[0]);
		$(this).parent().prev().find('.low_source').attr('src',src);
		$(this).parent().find('input[class="type-file-text"]').val($(this).val());
	});
