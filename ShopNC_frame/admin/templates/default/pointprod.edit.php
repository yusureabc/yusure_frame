  ...
  <form id="pointprod_form" method="post" enctype="multipart/form-data" >
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="noborder">
          <th rowspan="6" class="picture"><div class="size-200x200"><span class="thumb size-200x200"><i></i><img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_POINTPROD.DS.$output['prod_info']['pgoods_image'];?>" onerror="this.src='<?php echo UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.C('default_goods_image');?>'" onload="javascript:DrawImage(this,200,200);" nc_type="goods_image" /></span></div>
          </th>
      ...
      <tbody>
        <tr>
          <td colspan="10" class="required"><label><?php echo $lang['admin_pointprod_freightprice']; ?>:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
          <!--  固定运费 -->
            <input type="radio" <?php if ( ! $output['prod_info']['ptransport_id'] ) { echo 'checked'; } ?> name="freight" id="freight_1" value="0" onclick="span_freight();"/>
            <label for="freight_1"><?php echo $lang['admin_pointprod_fixed_freight']; ?></label>
            
          <!-- 使用运费模板 -->
            <input type="radio" <?php if ( $output['prod_info']['ptransport_id'] ) { echo 'checked'; } ?> name="freight" id="freight_0" value="1" onclick="freight_template();"/>
            <label for="freight_0"><?php echo $lang['admin_pointprod_freight_template']; ?></label>
            <input id="transport_id" type="hidden" value="<?php echo $output['prod_info']['ptransport_id'];?>" name="transport_id">
            <input id="transport_title" type="hidden" value="<?php echo $output['prod_info']['ptransport_title'];?>" name="transport_title">
          </td>
        ...
        <tr>
          <td class="vatop rowform"> 
          <span id='span_freight' <?php if ( $output['prod_info']['ptransport_id'] ) {?> style='display: none' <?php }?> >
          <!-- g_freight -->
          　<input id="g_freight" class="txt-short" style="width: 40px;" name="g_freight" value="<?php echo $output['prod_info']['pgoods_freightprice']; ?>" />　元　
          </span>

          <span <?php if ( ! $output['prod_info']['ptransport_id'] ) {?> style='display: none' <?php }?> class="transport-name" id="postageName">
            <?php echo $output['prod_info']['ptransport_title']; ?>
          </span>
          <span id="freight_template" <?php if ( ! $output['prod_info']['ptransport_id'] ) {?> style='display: none' <?php }?>>
            <a class="ncsc-btn" onclick="window.open( 'index.php?act=point_transport&op=point_transport_list&type=select' );" >选择运费模板</a>
          </span>
