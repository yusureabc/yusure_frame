          ...
          <td id="gcategory" colspan="8"><input type="hidden" id="cate_id" name="cate_id" value="" class="mls_id" />
            ...
            <select class="querySelect">
              ...
              <?php } ?>
            	...
            	<label> <?php echo $lang['is_open_distribute'];?></label>
            	<select class="querySelect" name="is_distribute">
          			<option value="0" <?php if (intval($output['search']['is_distribute'])==0 || $output['search']['is_distribute']==0) echo "selected" ;?> >请选择</option>
          			<option value="1" <?php if(intval($output['search']['is_distribute'])==1) echo "selected";?>><?php echo $lang['dis_is_open'];?></option>
          			<option value="2" <?php if(intval($output['search']['is_distribute'])==2) echo "selected";?>><?php echo $lang['dis_is_off']?></option>
          		</select>
            </td>
        </tr>
        <tr>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="align-center"><?php echo $output['state'][$v['goods_state']].$output['shake'][$v['is_shake']] ;?></td>
          ...
          <td class="align-center" style ="width:100px">
            ...
            <p><a href="javascript:void(0);" onclick="goods_lockup(<?php echo $v['goods_commonid'];?>);">违规下架</a></p>
            <p><a href="index.php?act=comment_collect&op=comment&goods_id=<?php echo $output['storage_array'][$v['goods_commonid']]['goods_id'];?>&goods_name=<?php echo $v['goods_name'];?>" >增加评论</a></p>
          	<?php if($v['goods_verify']==1 && $v['is_distribute']==null){?>
          	 <p><a href="index.php?act=distribute&op=dis_goods_setting&flag=<?php echo $v['goods_commonid'];?>" >设置分销</a></p>
          	<?php }else if($v['goods_verify']==1 && $v['is_distribute']!=null && $v['is_distribute']!=2){?>
          	 <p><a href="index.php?act=distribute&op=dis_edit_goods&flag=<?php echo $v['goods_commonid'];?>" >修改分销</a></p>
          	<?php }else if($v['goods_verify']==1 && $v['is_distribute']==2){?>
          	 <p><a href="index.php?act=distribute&op=dis_edit_goods&flag=<?php echo $v['goods_commonid'];?>" ><font color="red">开启分销</font></a></p>
          	<?php }?>

            <?php if( $v['is_balance'] == 0 ){?>
            <p><a href="index.php?act=goods&op=goods_balance&goods_commonid=<?php echo $v['goods_commonid'];?>&is_balance=1" >开启礼品卡支付</a></p>
            <?php } else if( $v['is_balance'] == 1 ){?>
            <p><a href="index.php?act=goods&op=goods_balance&goods_commonid=<?php echo $v['goods_commonid'];?>&is_balance=0" ><font color="orange">关闭礼品卡支付</font></a></p>
            <?php } ?>

          	<?php if($output['base_flag']==1){?>
          	 <p><a href="index.php?act=goods&op=set_goods_info&flag=<?php echo $v['goods_commonid'];?>" ><font color="orange">销量基数</font></a></p>
          	<?php }?>
          </td>
        </tr>
                ...
                $.getJSON('index.php?act=goods&op=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
                        ...
                        $.each(date, function(i, o){
                            $('<li><div class="goods-thumb" title="商家货号：' + o.goods_serial + '"><a href="' + o.url + '" target="_blank"><image src="' + o.goods_image + '" ></a></div>' + o.goods_spec + '<div class="goods-price">价格：<em title="￥' + o.goods_price + '">￥' + o.goods_price + '</em></div>' + o.shake_price + '<div class="goods-storage">库存：<em title="' + o.goods_storage + '">' + o.goods_storage + '</em></div><a href="' + o.url + '" target="_blank" class="ncsc-btn-mini">查看商品详情</a><br /><a id="shake_' + o.goods_id + '" href="javascript:void(0);" onclick="is_shake(' + o.goods_id + "," + _commonid + ');" >' + o.shake_color + '</a></li>').appendTo(_ul);
                            });
...
// 设置摇一摇
function is_shake(ids , commonid) {
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?act=goods&op=is_shake&id=" + ids + "&commonid=" + commonid ;
    CUR_DIALOG = ajax_form('is_shake', '设置摇一摇', _uri, 350);
}
</script> 
