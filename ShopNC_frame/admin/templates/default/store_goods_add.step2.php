...
<script src="<?php echo RESOURCE_SITE_URL;?>/js/sea_goods/store_goods_add.step2.js"></script>
<link href="<?php echo ADMIN_TEMPLATES_URL;?>/css/sea_addgoods.css" rel="stylesheet" type="text/css">
<link href="<?php echo ADMIN_TEMPLATES_URL;?>/css/font/font-awesome/css/sea-font-awesome.min.css" rel="stylesheet" type="text/css">
...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlAdmin('sea_goods_online', 'edit_save_goods');} else { echo urlAdmin('sea_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        ...
        <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_name'];?> <a class="ncsc-btn" href="<?php if ($output['edit_goods_sign']) { echo urlAdmin('sea_goods_online', 'edit_class', array('commonid' => $output['goods']['goods_commonid'], 'ref_url' => getReferer())); } else { echo urlAdmin('sea_goods_add', 'add_step_one'); }?>"><?php echo $lang['nc_edit'];?></a>
      ...
      <dl>
        <dt nc_type="no_spec"><i class="required">*</i><?php echo '商品批发价'.$lang['nc_colon'];?></dt>
        <dd nc_type="no_spec">
          ...
          <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，且不能高于零售价价。<br>此价格为商品实际销售价格，如果商品存在规格，该价格显示最低价格。</p>
        </dd>
      </dl>
      <dl>
        <dt><i class="required">*</i>零售价<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，此价格仅为参考售价，请根据该实际情况认真填写。</p>
        </dd>
      </dl>

      <?php
    		//修改DEFAULT_SPEC_COLOR_ID为数组
    		$spec_model = Model('sea_spec');
    		$spec_color_id_arr = $spec_model->specList(array('sp_name'=>'颜色'),'','sp_id');
    		$default_spec_color_id_arr = array();
    		foreach($spec_color_id_arr as $k=>$v) {
    		  $default_spec_color_id_arr[]=$v['sp_id'];
    		}
  		?>
      ...
      <!-- 规格 -->
      <dl nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if(in_array($k,$default_spec_color_id_arr)){?>spec_img="t"<?php }?>>
        <dt>
        ...
        <dd <?php if(in_array($k,$default_spec_color_id_arr)){?>nctype="sp_group_val"<?php }?>>
          <ul class="spec">
            <?php if(is_array($val['value'])){?>
            <?php foreach ($val['value'] as $v) {?>
            <li><span nctype="input_checkbox">
              <input type="checkbox" value="<?php echo $v['sp_value_name'];?>" nc_type="<?php echo $v['sp_value_id'];?>" <?php if(in_array($k,$default_spec_color_id_arr)){?>class="sp_val"<?php }?> name="sp_val[<?php echo $k;?>][<?php echo $v['sp_value_id']?>]">
              </span><span nctype="pv_name"><?php echo $v['sp_value_name'];?></span></li>
            <?php }?>
            <?php }?>
            <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo urlAdmin('sea_goods_add', 'ajax_add_spec');?>'}">
              ...
              <div nctype="specAdd2" style="display:none;">
                ...
                <a href="javascript:void(0);" nctype="specAddSubmit" class="ncsc-btn ncsc-btn-acidblue ml5 mr5">确认</a>
                <a href="javascript:void(0);" nctype="specAddCancel" class="ncsc-btn ncsc-btn-orange">取消</a>
              </div>
            </li>
          </ul>
        </dd>
        ...
        <dd class="spec-dd">
          <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
            ...
            <th nctype="spec_name_<?php echo $k;?>"><?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?></th>
              ...
              <th class="w120" style="display:none" id="th_integrate"><?php echo $lang['store_goods_step2_integrate_price'];?></th>
              <th class="w60" style="display:none" id="th_integrate_amount"><?php echo $lang['store_goods_step2_integrate_amount'];?></th>
                </thead>
        ...
        <dd>
          <div class="ncsc-goods-default-pic">
            <div class="goodspic-uplaod">
              <div class="upload-thumb"> <img nctype="goods_image" src="<?php echo sea_thumb($output['goods'], 240);?>"/> </div>
              ...
              <div class="handle">
                ...
                <a class="ncsc-btn" nctype="show_image" href="<?php echo urlAdmin('sea_album', 'pic_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>从图片空间选择</a> <a href="javascript:void(0);" nctype="del_goods_demo" class="ncsc-btn ml5" style="display: none;"><i class="icon-circle-arrow-up"></i>关闭相册</a></div>
            </div>
          </div>
          <div id="demo"></div>
        </dd>
      ...
      <dl>
        ...
        <dd>
          <span class="mr30">
            <a href="javascript:void(0);" class="ncsc-btn" id="goodsAttr"><i class="icon-plus"></i>添加商品属性</a>  
            <span></span>
              <!-- <span><br /><input type="text" name="goods_attr_name[]" placeholder='属性名' /><input type="text" name="goods_attr_value[]" placeholder='属性值' /><a href="javascript:void(0);" class="btn-red" nctype="goods_attr_del" ><i class="icon-trash"></i></a></span>        -->
              <?php if ( $goods_attr = unserialize( $output['goods']['goods_attr'] ) ) { ?>
                <?php foreach ( $goods_attr as $k => $v ) { ?>
                <span><br /><input type="text" name="goods_attr_name[]" placeholder='属性名' value="<?php echo $v['name']; ?>" /><input type="text" name="goods_attr_value[]" placeholder='属性值' value="<?php echo $v['value']; ?>" /><a href="javascript:void(0);" class="btn-red" nctype="goods_attr_del" ><i class="icon-trash"></i></a></span>
              <?php } } ?>
          </span>
        </dd>
      </dl>
      ...
      <dl>
        ...
        <dd>
          <?php showEditor('g_body',$output['goods']['goods_body'],'100%','480px','visibility:hidden;',"false",true);?>
          <div class="hr8"><div class="ncsc-upload-btn"> <a href="javascript:void(0);"><span>
                  ...
                  <a class="ncsc-btn" nctype="show_desc" href="index.php?act=sea_album&op=pic_list&item=des"><i class="icon-picture"></i><?php echo $lang['store_goods_album_insert_users_photo'];?></a> <a href="javascript:void(0);" nctype="del_desc" class="ncsc-btn ml5" style="display: none;"><i class=" icon-circle-arrow-up"></i>关闭相册</a> </p>
          <p id="des_demo"></p>
        </dd>
      </dl>
      ...
      <dl>
        <dt><?php echo '仓库'.$lang['nc_colon']?></dt>
        <dd>
          <p id="house">
          <?php foreach ( $output['house_list'] as $k => $v ) { ?>
            <input type="radio" name="house_id" <?php echo $output['goods']['house_id'] == $v['house_id'] ? 'checked' : ''; ?> value="<?php echo $v['house_id']; ?>" /> <?php echo $v['house_name']; ?>　
          <?php } ?>
            <span></span>
          </p>
        </dd>
          ...
          <ul class="ncsc-form-radio-list">
            <li>
              ...
              <div nctype="div_freight" <?php if (intval($output['goods']['transport_id']) != 0) {?>style="display: none;"<?php }?>>
                <input id="g_freight" class="w60 text" nc_type='transport' type="text" value="<?php printf('%.2f', floatval($output['goods']['goods_freight']));?>" name="g_freight"><em class="add-on"><i class="icon-renminbi"></i></em>
              </div>
            </li>
            <li>
      ...
      <dl>
        <dt><?php echo '税率'.$lang['nc_colon'];?></dt>
        <dd>
          <input name="tax_rate" type="text" class="text w60" value="<?php echo $output['goods']['tax_rate']; ?>" />　%
          <span></span>
          <p class="hint"> <?php echo $lang['tax_rate_help_1']; ?> </p>
        </dd>
      </dl>

      <dl>
        <dt><?php echo '免税额'.$lang['nc_colon'];?></dt>
        <dd>
          <input name="rate_quota" type="text" class="text w60" value="<?php echo $output['goods']['rate_quota'] ? : 50; ?>" /><em class="add-on"><i class="icon-renminbi"></i></em>
          <p class="hint"> <?php echo $lang['rate_quota_help_1']; ?> </p>
        </dd>
      </dl>
    </div>
...
$(function(){
    ...
    $('#goods_form').validate({
        ...
        messages : {
            ...
            house_id : {
                required    : '<i class="icon-exclamation-sign"></i>请选择商品仓库'
            },
            tax_rate : {
                required    : '<i class="icon-exclamation-sign"></i>请填写商品税率',
                number      : '<i class="icon-exclamation-sign"></i>请填写数字格式的税率',
            }
        }
    });
...
$(function(){
  /* 点击规格时 */
	...
	$("input[name=is_integrate]").on('change',function(){
		var nc_len = $("span[nctype='input_checkbox']").length;
		if(nc_len>0){
			into_array();
			goods_stock_set();
		}
...
function goods_stock_set(){
    ...
    // $('input[name="g_integrate_price"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    // $('input[name="g_integrate"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    $('dl[nc_type="spec_dl"]').show();
    str = '<tr>';
   

		$("#th_integrate").hide();
		$("#th_integrate_amount").hide();
    ...
    if(str == '<tr>'){
        ...
        $('input[name="g_integrate_price"]').removeAttr('readonly').css('background','');
        $('input[name="g_integrate"]').removeAttr('readonly').css('background','');
        ...
        $('tbody[nc_type="spec_table"]').empty().html(str)
            ...
            .find('input[data_type="price"]').change(function(){
             ...
             .find('input[data_type="in_price"]').change(function(){
            	 computeInPrice();     //积分价格计算
            }).end()
            .find('input[data_type="in_amount"]').change(function(){
            	 computeInAmount();     // 积分个数计算
            }).end()
...
function recursionSpec($len,$sign,$type=false) {
  $temp_str = "" ;
    if($len < $sign){
        ...
        if($type){
          $temp_str =  "<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][in_price]\" data_type=\"in_price\"  ".
                       " nc_type=\"'+spec_bunch+'|in_price\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td>".
                 "<td><input class=\"text\" type=\"text\" name=\"spec['+spec_bunch+'][in_amount]\" data_type=\"in_amount\"  ".
                       " nc_type=\"'+spec_bunch+'|in_amount\" value=\"\" /></td>";
          
        }
        echo "str +='<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][price]\" data_type=\"price\"  ".
           " nc_type=\"'+spec_bunch+'|price\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td>".
...
$(function(){
    ...
    str = '<tr>';
    <?php if($output['goods']['is_integrate_goods']>0){
			 recursionSpec(0,$output['sign_i'],true);
    }else{
    	 recursionSpec(0,$output['sign_i']);
    }?>
	
    <?php if($output['goods']['is_integrate_goods']>0){?>
    	$("#th_integrate").show();
    	$("#th_integrate_amount").show();
    <?php }else{?>
    	$("#th_integrate").hide();
    	$("#th_integrate_amount").hide();
    <?php }?>
...
function computeInPrice(){
    // 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="in_price"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
    ...
    $('input[name="g_integrate_price"]').val(number_format(_price, 2));

}

//计算最低积分
function computeInAmount(){
    var _price = 0;var _price_sign = false;
    $('input[data_type="in_amount"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
    ...
    $('input[name="g_integrate"]').val(_price);
}
</script>