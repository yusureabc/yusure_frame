  ...
  <table class="table tb-type2 nobdb">
    <thead>
      <tr class="thead">
        ...
        <th class="align-center"><?php echo $lang['order_state'];?></th>
         <th class="align-center"><?php echo $lang['order_originate'];?></th>
    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <td class="align-center"><?php echo orderState($order) . order_shake($order['app_shake']) . order_others($order['order_others']);?></td>
         <td style="text-align:center;">
        	<?php if($order['order_originate']=="computer"){ echo "电脑端";}elseif($order['order_originate']=="phone"){ echo "手机端";}else{
        		echo "暂未确定";
        	}?>
        </td>
        <!--编辑标识-->
         <td style="text-align:center;"><a><span class="fcolor"><img id="image_<?php echo $order['order_id']; ?>" src="<?php echo ADMIN_TEMPLATES_URL.DS;?>/images/<?php if(isset($order['flag_color'])){echo $order['flag_color'];}else{echo '0';}?>.png"/></span>
        <select  id="select_color_<?php echo $order['order_id']; ?>" name="select_color" onchange="sendRequest( <?php echo $order['order_id']; ?> )"><!--sendRequest()-->
        <option  value ="0" <?php if($order['flag_color']=='0'){echo selected;}?>>默认</option>
        <option  value ="1" <?php if($order['flag_color']=='1'){echo selected;}?>>红色</option>
        <option value ="2" <?php if($order['flag_color']=='2'){echo selected;}?>>黄色</option>
        <option value="3" <?php if($order['flag_color']=='3'){echo selected;}?>>绿色</option>
        <option value="4" <?php if($order['flag_color']=='4'){echo selected;}?>>蓝色</option>
        <option value="5" <?php if($order['flag_color']=='5'){echo selected;}?>>粉色</option></select></a></td>
...
<script>
 var xmlHttp=null;
  ...
  function sendRequest( order_id ){
    var flag_color=document.getElementById( 'select_color_' + order_id ).value;
    if(flag_color==null || flag_color=="")
      return;
    ...
    var url = "index.php?act=order&op=editflag&order_id=" + order_id + "&flagcolor="+flag_color;
    xmlHttp.open("Get",url,true);
    xmlHttp.onreadystatechange = function()
    {
        if ( xmlHttp.readyState==4 && xmlHttp.status==200 ) {
          var response = xmlHttp.responseText;
         document.getElementById( 'image_' + order_id ).src = response;
        }
    }
    xmlHttp.send( null );
  }
