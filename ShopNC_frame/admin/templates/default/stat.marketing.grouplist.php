...
<div style="border:1px #D9D9D9 solid; padding:10px; width:98%;">
    ...
    <table class="table tb-type2 nobdb">
        ...
        <?php foreach ((array)$output['grouplist'] as $k=>$v){?>
          <tr nc_type="gitem" class="hover item">
            <td><?php echo $v['groupbuy_name'];?></td>
            <td>团购时间：<?php echo @date('Y-m-d',$v['start_time']).'~'.@date('Y-m-d',$v['end_time']);?></td>
            ...
            <td colspan="3">
            	...
            	<div class="left">
                	...
                	<div class="close_float">
                		...
                		<span class="infoitem left">购买量：<em class="red_common"><?php echo $v['buy_quantity'];?></em></span>
                		<span class="infoitem left" style="width:200px;">总金额：<em class="red_common"><?php echo ncPriceFormat($v['buy_quantity']*$v['groupbuy_price']).$lang['currency_zh'];?></em></span>
                		<span class="infoitem left" style="width:300px;">下单转化率：<em class="red_common"><?php echo $v['views']>0?round($v['buyer_count']/$v['views']*100,2):0;?>%</em></span>
                	</div>
            	</div>
            </td>
          </tr>
