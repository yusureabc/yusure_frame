      ...
      <tbody>
        ...
        <tr class="hover edit <?php echo getStoreStateClassName($v);?>">
          <td>
              <a href="<?php echo urlShop('show_store','index', array('store_id'=>$v['store_id']));?>" target="_blank">
                ...
                <?php if($v['store_id'] == DEFAULT_PLATFORM_STORE_ID) {echo '(平台)';}?>
          	</a></td>
        ...
        <td class="align-center w120">
            <a href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>">查看</a>&nbsp;&nbsp;<a href="index.php?act=store&op=store_edit&store_id=<?php echo $v['store_id']?>"><?php echo $lang['nc_edit'];?></a>&nbsp;&nbsp;
                ...
                <?php } ?>
                <?php if($v['the_store']==1){?>
                	<a href="index.php?act=store&op=edit_distribute_store&store_id=<?php echo $v['store_id']?>">修改门店系统</a>
                <?php }else{?> 
                	<a href="index.php?act=store&op=add_distribute_store&store_id=<?php echo $v['store_id']?>"><font color="red">开启门店系统</font></a>
                <?php }?>
                &nbsp;  <a href="index.php?act=store&op=store_genqrcode&store_id=<?php echo $v['store_id'];?>"><span style="color:orange">生成二维码</span></a>
            </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
