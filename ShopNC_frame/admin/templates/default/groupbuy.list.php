      ...
      <tbody id="treet1">
        ...
        <tr class="hover">
          ...
          <td class="group"><p><a target="_blank" href="<?php echo SHOP_SITE_URL."/index.php?act=show_groupbuy&op=groupbuy_detail&group_id=".$val['groupbuy_id'];?>"><?php echo $val['groupbuy_name'];?></a></p>
            ...
            <p class="store"><?php echo $lang['groupbuy_index_store_name'];?>:<a target="_blank" href="<?php echo urlShop('show_store','index', array('store_id'=>$val['store_id']));?>" title="<?php echo $val['store_name'];?>"><?php echo $val['store_name'];?></a></p></td>
        ...
        <td class="align-center">
            ...
            <a nctype="btn_refresh" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:;">刷新</a>
        </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('[nctype="btn_refresh"]').on('click', function() {
            if(confirm('您将根据当前团购状态刷新商品锁定情况！')) {
                var action = '<?php echo urlAdmin('groupbuy', 'goodslock_refresh');?>';
                var groupbuy_id = $(this).attr('data-groupbuy-id');
                $('#op_form').attr('action', action);
                $('#groupbuy_id').val(groupbuy_id);
                $('#op_form').submit();
