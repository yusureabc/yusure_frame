...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.inline_edit').blur(function(){
            ...
            $.getJSON('index.php?act=merchant&op=ajax_update_class&type='+type+'&class_id='+class_id+'&value='+value, function(result){
...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        ...
        $('#list_form').attr('action','index.php?act=merchant&op=drop');
        $('#class_id').val(id);
        $('#list_form').submit();
    }
}
function ajax_set_recommend(stat,class_id){
	$.getJSON('index.php?act=merchant&op=ajax_recommend&class_id='+class_id+'&stat='+stat, function(result){
		if(result.done){
			if(stat == 1){
				var stat_show = '是';
  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
		...
		<li><a href="index.php?act=merchant&op=add"><span><?php echo $lang['nc_add'];?></span></a></li>
      </ul>
    </div>
  </div>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="name">
          <input type="text" edit_type="class_name" value="<?php echo $val['class_name'];?>" oldvalue="<?php echo $val['class_name'];?>" class="inline_edit" class_id="<?php echo $val['class_id']; ?>" /> </td>
          ...
          <td class="align-center"><a href="index.php?act=merchant&op=edit&class_id=<?php echo $val['class_id'];?>"><?php echo $lang['nc_edit'];?></a> | <a href="javascript:submit_delete(<?php echo $val['class_id'];?>)"><?php echo $lang['nc_del'];?></a></td>
        </tr>
        ...
        <tr class="hover edit <?php echo 'class_id'.$val['class_id'];?>" style="display:none;">
          ...
          <td class="w200 align-center">
              <a href="index.php?act=merchant&op=edit&class_id=<?php echo $val1['class_id'];?>"><?php echo $lang['nc_edit'];?></a> | 
