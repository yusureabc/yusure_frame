...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        ...
        $('#list_form').attr('action','index.php?act=mb_cf&op=mb_cf_drop');
        $('#class_id').val(id);
        $('#list_form').submit();
    }
}
function submit_update(id)
{
  ...
  $('#list_form').attr('method','post');
  $('#list_form').attr('action','index.php?act=mb_cf&op=mb_cf_update');
  $('#class_id').val(id);
  $('#list_form').submit();
}
</script>
...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3><?php echo $lang['nc_mobile_classification'];?></h3>
      <ul class="tab-base">
  ...
  <form id="list_form" method='post'>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          <td class="w24"><input type="checkbox" value="<?php echo $val['cf_id'];?>" class="checkitem"></td>
          <td class="w48 sort"><span nc_type="class_sort" column_id="<?php echo $val['cf_id'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable "><?php echo $val['cf_sort'];?></span></td>
          <td class="name"><span nc_type="class_name" column_id="<?php echo $val['cf_id'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable "><?php echo $val['cf_name'];?></span></td>
          <td class="w72 align-center">
            <a href="javascript:submit_update(<?php echo $val['cf_id'];?>)"><?php echo $lang['nc_edit'];?></a> | 
            <a href="javascript:submit_delete(<?php echo $val['cf_id'];?>)"><?php echo $lang['nc_del'];?></a>
          </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
