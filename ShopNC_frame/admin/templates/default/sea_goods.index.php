...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="<?php echo urlAdmin( 'sea_goods', 'goods', array('type' => 'lockup'));?>" ><span> <?php echo $lang['goods_index_off_goods']; ?> </span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="get" name="formSearch" id="formSearch">
    <input type="hidden" name="act" value="sea_goods">
  ...
  <form method='post' id="form_goods" action="<?php echo urlAdmin('sea_goods', 'goods_del');?>">
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w60 picture"><div class="size-56x56"><span class="thumb size-56x56"><i></i><img src="<?php echo sea_thumb($v, 60);?>" onload="javascript:DrawImage(this,56,56);"/></span></div></td>
          ...
          <td class="align-center">
            <p><a href="<?php echo urlOverseas('overseas_goods_detail', 'goods_detail_info', array('goods_id' => $output['storage_array'][$v['goods_commonid']]['goods_id']));?>" target="_blank"><?php echo $lang['nc_view'];?></a></p>
            <p><a href="javascript:void(0);" onclick="goods_lockup(<?php echo $v['goods_commonid'];?>);">下架</a></p>
            <p><a href="<?php echo urlAdmin('sea_goods_online', 'edit_goods', array('commonid' => $v['goods_commonid']));?>"><?php echo $lang['nc_edit'];?></a></p>
          </td>
        </tr>
    ...
    $('i[nctype="ajaxGoodsList"]').toggle(
        function(){
          // 打开
            ...
            if (_div.html() == '') {
                $.getJSON('index.php?act=sea_goods&op=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {

                        var _ul = $('<ul class="ncsc-goods-sku-list"></ul>');
                        $.each(date, function(i, o){
                            $('<li><div class="goods-thumb" title="商家货号：' + o.goods_serial + '"><a href="' + o.url + '" target="_blank"><image src="' + o.goods_image + '" ></a></div>' + o.goods_spec + '<div class="goods-price">价格：<em title="￥' + o.goods_price + '">￥' + o.goods_price + '</em></div>'+'<div class="goods-storage">库存：<em title="' + o.goods_storage + '">' + o.goods_storage + '</em></div><a href="' + o.url + '" target="_blank" class="ncsc-btn-mini">查看商品详情</a><br /></li>').appendTo(_ul);
                            });
...
function goods_lockup(ids) {
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?act=sea_goods&op=goods_lockup&id=" + ids;
