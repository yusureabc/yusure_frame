...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_joke_rec'];?>')) {
        $('#list_form').attr('method','post');
        $('#list_form').attr('action','index.php?act=shake_joke&op=joke_rec');
        $('#joke_id').val(id);
        $('#list_form').submit();
    }
}
...
{
  $('#list_form').attr('method','post');
  $('#list_form').attr('action','index.php?act=shake_joke&op=joke_edit');
  $('#joke_id').val(id);
  $('#list_form').submit();
}
</script>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w72 align-center">
            <a href="javascript:submit_update(<?php echo $val['joke_id'];?>)"><?php echo $lang['nc_edit'];?></a> | 
            <a href="javascript:submit_delete(<?php echo $val['joke_id'];?>)"><?php echo $lang['nc_rec'];?></a>
          </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
