  ...
  <form id="goods_class_form" name="goodsClassForm" enctype="multipart/form-data" method="post">
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <?php } ?>
        <tr>
            <td colspan="2"><label for="pic"><?php echo '发布虚拟商品';?>:</label></td>
        </tr>
        <tr class="noborder">
            <td class="vatop rowform onoff">
                <label for="is_virtul1" class="cb-enable <?php if($output['class_array']['is_virtul'] == '1'){ ?>selected<?php } ?>"><span>是</span></label>
                <label for="is_virtul0" class="cb-disable <?php if($output['class_array']['is_virtul'] == '0'){ ?>selected<?php } ?>"><span>否</span></label>
                <input id="is_virtul1" name="is_virtul" <?php if($output['class_array']['is_virtul'] == '1'){ ?>checked="checked"<?php } ?> value="1" type="radio">
                <input id="is_virtul0" name="is_virtul" <?php if($output['class_array']['is_virtul'] == '0'){ ?>checked="checked"<?php } ?> value="0" type="radio">
                <div class=" mtm"><input type="checkbox" name="vt_associated" value="1"  id="vt_associated" /><label for="t_associated"><?php echo $lang['goods_class_edit_related_to_subclass'];?></label></div>
            </td>
            <td class="vatop tips"><?php echo '允许发布虚拟商品后，在该分类发布商品时可选择交易类型为“虚拟兑换码”形式';?></td>
        </tr>
      <!-- 海外购 -->
        <tr>
            <td colspan="2"><label for="pic"><?php echo '海外购';?>:</label></td>
        </tr>
        <tr class="noborder">
            <td class="vatop rowform onoff">
                <label for="is_oversea1" class="cb-enable <?php if($output['class_array']['is_oversea'] == '1'){ ?>selected<?php } ?>"><span>是</span></label>
                <label for="is_oversea0" class="cb-disable <?php if($output['class_array']['is_oversea'] == '0'){ ?>selected<?php } ?>"><span>否</span></label>
                <input id="is_oversea1" name="is_oversea" <?php if($output['class_array']['is_oversea'] == '1'){ ?>checked="checked"<?php } ?> value="1" type="radio">
                <input id="is_oversea0" name="is_oversea" <?php if($output['class_array']['is_oversea'] == '0'){ ?>checked="checked"<?php } ?> value="0" type="radio">          
            </td>
            <td class="vatop tips"><?php echo '开启海外购后，该大分类下的所有小分类都为海外购';?></td>
        </tr>
