	...
	protected function send_notice($receiver_id,$tpl_code,$param,$flag = true){
		...
		switch($mail_tpl['type']){
			case '0':
				$email	= new Email();
				$result	= true;
				if($GLOBALS['setting_config']['email_enabled'] == '1'){
...
class BaseHomeControl extends Control {
	...
	public function __construct(){
		...
		$this->queryCart();
		$sM = Model("setting");
		$set_con['name'] = "trans_money_name";
		$res = $sM->where($set_con)->find();
		if($res){
			Tpl::output("tags",$res['value']);
		}else{
			Tpl::output("tags","金币");
		}
...
class BaseMemberControl extends Control {
	public function __construct(){
		...
		$nav_link = array(
			...
			array(
				'title'=>Language::get('nc_user_center'),
				'link'=>urlShop('member', 'home')
			),
			//array(
			//	'title'=>$lang['nc_member_path_'.$output['menu_sign']]
			//)
		);
		...
		// 是否分销商
		$distribute_mem_M = Model( 'distribute_member' );
		$dis_con['member_id'] = $_SESSION['member_id'];
		$dis_con['is_valid']  = 1;
		$dismember_res = $distribute_mem_M->getOneInfo( $dis_con );
		if ( $dismember_res )
		{
			Tpl::output( 'distribute', $dismember_res );
		}
		if($_SESSION['is_community']=="1"){
			Tpl::output( 'is_o2o', "yes" );
		}
		$this->article();
	}
...
class BaseStoreControl extends Control {
	...
	public function __construct(){
        ...
        if($store_info['store_theme']=="super6"){
        	 Tpl::output('store_banner_show', 1);//如果选择模版super6 则 前台不显示banner  农村电商模版 运营部要求 （蒋亚楠）
        }
        //店铺装修
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        if ($_GET['act'] !== 'seller_login' && $_GET['act'] !== 'pconnect') {
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                ...
                array('name' => '分销商品设置','act'=>'store_goods_distribute','op'=>'index'),
            ...
            'order' => array('name' => '订单', 'child' => array(
                ...
                array('name' => '虚拟订单', 'act'=>'store_order_virtual', 'op'=>'index'),
            ...
            'account' => array('name' => '帐号', 'child' => array(
				...
				array('name' => '短信配置', 'act'=>'store_sms_conf', 'op'=>'index'),
            )),
            'staff' => array('name' => '门店', 'child' => array(
            	array('name' => '员工列表', 'act'=>'store_staff', 'op'=>'staff_list'),
            	array('name' => '短信充值', 'act'=>'msg_receive', 'op'=>'msg_list'),
            	array('name' => '客户管理', 'act'=>'store_member', 'op'=>'member_list'),
        	))
        );
        return $menu_list;
    }
 ...
 * 社区O2O的基类  
 */
class memberstoreControl extends BaseMemberControl{
	...
	public function __construct(){
		parent::__construct();
		Language::read('o2o_common');
		Language::read('o2o_member_store');
		if(!isset($_SESSION['is_community']) || $_SESSION['o2o_storeid']<1){
		 ...
		 * 设置模板文件夹路径
		 */
		Tpl::setDir('O2O_member');
		/**
		 * 设置布局文件内容
		 */
		Tpl::setLayout('O2O_member/member_store_layout');
		$this->layout	=	'O2O_member/member_store_msg';
		$storeinfo = $this->getStoreInfo($_SESSION['o2o_storeid']);
		$this->store = $storeinfo;
		if($storeinfo['store_state'] == 1){			
			$this->showTip(L('nc_member_store_now_apply'),'index.php?act=comm_storedetail');
	 ...
	 */
	protected function getStoreInfo($store_id){
		$store_model = Model('o2o_merchant');
...
class baseo2ostoreControl extends BaseMemberControl{
	/**
	 * 构造函数
	 */
	public function __construct(){
		 ...
		 */
		Language::read('o2o_common');
		Language::read('o2o_store');
		Language::read('o2o_home');
		 ...
		 * 设置布局文件内容
		 */
		Tpl::setLayout('O2O_member/merchant_layout');
		
		//当前登录用户的session信息而非商铺
		if($_SESSION['member_id']){
			$this->member_info = $_SESSION;
			Tpl::output('member_o2o',$this->member_info);
		}
		...
		if(empty($_COOKIE['city'])){		
			$areaM = Model("o2o_area");
			$area_con['area_id'] = O2O_DEFAULT_CIYT;
			$city_info = $areaM->where($area_con)->find();
			$this->city_info = $cityinfo;
			$area_str = serialize($city_info);
...
class BaseCommunityControl extends Control{
	...
	public function __construct(){
		...
		//关于默认城市的选择
		if(empty($_COOKIE['city'])){//默认选择的是青岛，这个在b2b2c.php中设置	
			$areaM = Model("o2o_area");
			$area_con['area_id'] = O2O_DEFAULT_CIYT;
			$cityinfo = $areaM->where($area_con)->find();
			$this->city_info = $cityinfo;
			$area_str = serialize($city_info);
			setcookie('city',$area_str,time()+3600*24*30);
		}else{
			$cityinfo = unserialize(stripslashes($_COOKIE['city']));
			if(!$cityinfo){//之前出现了$_COOKIE['city']反序列化之后是空的情况，若是空的话走下面
				$areaM = Model("o2o_area");
				$area_con['area_id'] = O2O_DEFAULT_CIYT;
				$cityinfo = $areaM->where($area_con)->find();
				$this->city_info = $cityinfo;
	...
	protected function classlist(){
		if(!$class = F('class',null,'cache/class')){
			$model_class = Model('o2o_merchant_class');
			...
			$class = array('class_root'=>$class_root);
			F('class',$class,'cache/class');
		}
