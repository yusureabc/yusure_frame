...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo '<dd><p /><p /><p /><p /></dd>';
			//echo '<dd><p /><p /><p /><p />Copyright 2007-2014 All Rights Reserved '.$lang['company_name'].'</dd>';
			echo '</dl>';
			echo '</body>';
			echo '</html>';
			exit;
...
function getMemberAvatarForID($id){
	...
	if($id){
		$member_avatar = $member_db->getfby_member_id($id,'member_avatar');
		if(!$member_avatar){
			return UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait');exit;
		}
...
function write_file($filepath, $data, $mode = null){
	...
	if ($data === '') $data = '\'\'';
	$data = "<?php defined('IN_B2B2C') or exit('Access Invalid!'); return ".$data."\n?>";
...
function H($key, $value='', $cache_type='', $expire=null, $args=null){
    ...
    if ($value !== '') {
        if (is_null($value)) { // 删除缓存
            ...
            $obj_cache->set($key, ($list?$list:var_export($value,true)), null, $expire);
 ...
 * @return array/bool
 */
function new_rcache($key = null, $prefix = '', $fields = '*'){
 ...
 * @return bool 返回值
 */
function new_wcache($key = null, $data = array(), $prefix, $period = 0){
...
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    $reg_match_from = array(
        ...
        '/^document-index-code-([a-z_]+)\.html$/',
       	...
       	'/^fxSearch-index-cate_id-(\d+)-b_id-([0-9_]+)-a_id-([0-9_]+)-key-([0-3])-order-([0-2])-type-([0-2])-area_id-(\d+)-curpage-(\d+)\.html$/',
		
       	'/^comm_index-index\.html$/',//o2o首页
    	'/^comm_index-detail-id-(\d+)-curpage-(\d+)\.html$/',//o2o商铺详情
    	'/^comm_goods-detail-goods_id-(\d+)\.html$/',//o2o商品详情
        ...
        '/^integrate-getIntegList-curpage-(\d+).html$/'
        );
    $reg_match_to = array(
        'item-\\1.html',
        'shop-\\1.html',
        ...
        'fenxiao-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8.html',
    	'o2ohome.html',
    	'o2omerchant-\\1-\\2.html',
    	'o2ogoods-\\1.html',
...
function urlShop($act = '', $op = '', $args = array(), $store_domain = ''){
    ...
    if (intval(C('enabled_subdomain')) == 1 && !empty($store_domain)){
    	$shop_domain = 'http://'.$store_domain.'.'.SUBDOMAIN_SUFFIX.'/';
    	if($act =='show_store' && $op == 'index') {
    		return $shop_domain;
    	}
    } else {
    	$shop_domain = SHOP_SITE_URL;
    }
	if($act=='show_store'  && $args['store_id'] && intval(C('enabled_subdomain')) == 1 && $store_domain=='') {
		$model_store = Model('store');
		$store_info = $model_store->getStoreInfo(array('store_id'=>intval($args['store_id'])));
		if($store_info['store_domain']) {
			$shop_domain = 'http://'.$store_info['store_domain'].'.'.SUBDOMAIN_SUFFIX2.'/';
			if($op == 'index') {
				return $shop_domain;
			}
		}
    ...
    $rewrite_item = array('goods:index', 'goods:comments_list', 'search:index', 'show_store:index', 'show_store:goods_all', 'article:show',
            ...
            'show_groupbuy:groupbuy_detail', 'pointprod:index', 'pointvoucher:index', 'pointprod:pinfo', 'pointprod:plist','fxSearch:index','integrate:getIntegList',
    		'comm_index:index','comm_index:detail','comm_goods:detail');
    if(URL_MODEL && in_array($act.':'.$op, $rewrite_item)) {
        ...
        switch ($act.':'.$op) {
            ...
            case 'goods:comments_list':
                ...
                break;
            case 'goods:index':
            	  if ( $_GET['pmemberid'] || $_GET['type'] || $args['type'] || $args['pmemberid'] ) {
                    $rewrite_flag = false;
                    
                }
            break;
            case 'integrate:getIntegList' :
            	 $tpl_args['curpage'] = empty($args['curpage']) ? 1 : $args['curpage'];
            break;
            case 'comm_index:detail':
            	$tpl_args['id'] = empty($args['id']) ? 0 : $args['id'];
            	$tpl_args['curpage'] = empty($args['curpage']) ? 1 : $args['curpage'];
            	$args = $tpl_args;
                break;
            default:
    ...
    return url($act, $op, $args, $rewrite_flag, $shop_domain);
}

/**
 * 海外购使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $act control文件名
 * @param string $op op方法名
 * @param array $args URL其它参数
 ...
 * @return string
 */
function urlOverseas($act = '', $op = '', $args = array(), $store_domain = ''){
    // 开启店铺二级域名
    $shop_domain = '';
    if (intval(C('enabled_subdomain')) == 1 && !empty($store_domain)){
        $shop_domain = 'http://'.$store_domain.'.'.SUBDOMAIN_SUFFIX.'/';
...
//返回分类对应的路径
function get_CategoryUrl($category_id,$id) {
	$cat_array=array("1"=>"商铺","2"=>"商品","3"=>"分类","4"=>"关键词","5"=>"网址");
	foreach($cat_array as $key=>$value) {
		if($key==$category_id) {
			return 'http://jump/'.$key.'#'.$id;
		}
	}
}
...
{ 
	...
	$str = str_replace( '美团', '高一品', $str );	// 20150922鞠老师反馈bug
 ...
 * 获取o2o商铺的图片
 * Enter description here ...
 * @param $store_arr   商铺信息的数组
 * @param $type		  业务流程区别符【o2o商铺列表有用到 - type=o2o_store ; 店铺详情页的商品 - type=o2o_goods】
 */
function o2oImg($arr = array(),$type){
	if(!$arr || !$type){
		return ;
	}
	$final_url = "";
	switch ($type) {
		case "o2o_store":
			if($arr['store_id']){
				$temp_url = BASE_UPLOAD_PATH.DS.ATTACH_STORE_PATH.DS.$arr['store_id'].
							 DS.$arr['logo'];
				if(!file_exists($temp_url)){
					$final_url = UPLOAD_SITE_URL.DS.ATTACH_STORE_PATH.DS."shopnopic.png" ;
				}else{
					$final_url =  UPLOAD_SITE_URL.DS.ATTACH_STORE_PATH.DS.$arr['store_id'].
							 DS.$arr['logo'];
				}
			}
		break;
		case "o2o_goods":
			if($arr['goods_pic']){
				$temp_pic_arr = explode(",", $arr['goods_pic']);
				$temp_url = BASE_UPLOAD_PATH.DS.ATTACH_GOODS_PATH.DS.$arr['store_id'].DS.$temp_pic_arr[0];
				if(file_exists($temp_url)){
					$final_url = UPLOAD_SITE_URL.DS.ATTACH_GOODS_PATH.DS.$arr['store_id'].
							 DS.$temp_pic_arr[0];
				}else{
					$final_url = UPLOAD_SITE_URL.DS.ATTACH_GOODS_PATH.DS."shopnopic.png";
				}
			}else{
				$final_url = UPLOAD_SITE_URL.DS.ATTACH_GOODS_PATH.DS."shopnopic.png";
			}
		...
		case 'store_info_pic':
			if ( $arr['pic'] )
			{
				$img_path = BASE_UPLOAD_PATH.DS.'/o2o/merchant/'.DS.$arr['store_id'].DS.$arr['pic'];
				if ( file_exists($img_path) )
				{
					$final_url = UPLOAD_SITE_URL.DS.'/o2o/merchant/'.DS.$arr['store_id'].DS.$arr['pic'];
				}
				else
				{
					$final_url = UPLOAD_SITE_URL.DS.ATTACH_GOODS_PATH.DS."shopnopic.png";
