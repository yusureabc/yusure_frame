...
final class Base{
	...
	private static function start_session(){
		if (SUBDOMAIN_SUFFIX){
            ...
            $subdomain_suffix = explode(':',$subdomain_suffix);
            $subdomain_suffix = $subdomain_suffix[0];
		}
		//session.name强制定制成PHPSESSID,不请允许更改
		@ini_set('session.name','PHPSESSID');
		$subdomain_suffix = str_replace('http://','',$subdomain_suffix);
		if ($subdomain_suffix !== 'localhost') {
		    @ini_set('session.cookie_domain', COOKIE_DOMAIN?COOKIE_DOMAIN:$subdomain_suffix);
		}
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://www.b2b2c.com');exit();
		}else{
