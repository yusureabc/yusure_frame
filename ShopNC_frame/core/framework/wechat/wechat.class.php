	...
	*/
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
			...
			H($authname,$this->access_token,C('cache.type'),$expire);
			return $this->access_token;
		}
		return false;
	}
	 ...
	 * @return boolean
	 */
	public function checkOrderSignature_wx($orderxml=''){
		if (!$orderxml) {
			// 获取微信传过来的成功数据, php://input  $_REQUEST 取得是数组
			$postStr = file_get_contents("php://input");
			if (!empty($postStr)) {
				$orderxml = (array)simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
			} else return false;
		}
		$arrdata = array('appid'=>$orderxml['AppId'],'appkey'=>$this->appkey,'timestamp'=>$orderxml['TimeStamp'],'noncestr'=>$orderxml['NonceStr'],'openid'=>$orderxml['OpenId'],'issubscribe'=>$orderxml['IsSubscribe']);
		$paySign = $this->getSignature($arrdata);
		if ($paySign!=$orderxml['AppSignature']) return false;
		return true;
	}
