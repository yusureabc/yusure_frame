...
class Page{
	...
	public function show($style = null){
		...
		switch ($this->style) {
			...
			case '5':
				...
				break;
			case '6':
				$this->left_current_html = '<li class="active"><span>';
				$html_page .= '<ul class="pagination">';
				if ($this->getNowPage() == 1){
					$html_page .= '<li>'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</li>';
					$html_page .= '<li>'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</li>';
				}else {
					$html_page .= '<li><a class="demo" href="'. $this->setOverseasPseudoStaticPageUrl('1') .'">'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</a></li>';
					$html_page .= '<li><a class="demo" href="'. $this->setOverseasPseudoStaticPageUrl($this->getNowPage()-1) .'">'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</a></li>';
				}
				$html_page .= $this->getNowBar();
				if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
					$html_page .= '<li class="next">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</li>';
					$html_page .= '<li class="prev">'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</li>';
				}else {
					$html_page .= '<li><a class="demo" href="'. $this->setOverseasPseudoStaticPageUrl($this->getNowPage()+1) .'">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</a></li>';
					$html_page .= '<li><a class="demo" href="'. $this->setOverseasPseudoStaticPageUrl($this->getTotalPage()) .'">'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</a></li>';
				}
				$html_page .= '</ul>';
				break;
			default:
     ...
     * @return string 字符串类型的返回结果
     */
    private function setOverseasPseudoStaticPageUrl($page){
        $param = $_GET;
        $act = $param['act'] == '' ? 'index' : $param['act'];
        unset($param['act']);
        $op = $param['op'] == '' ? 'index' : $param['op'];
        unset($param['op']);
        $param[$this->page_name] = $page;
        return urlOverseas($act, $op, $param);
    }
}
