 ...
 * 可以是移动手机还是平板
 * $mobile = new Mobile;
 * $mobileType = ($mobile->isMobile() ? ($mobile->isTablet() ? 'tablet' : 'phone') : 'computer');
 * $scriptVersion = $mobile->getScriptVersion(); 
 * $mobile->isMobile();  手机
 * $mobile->isMobile()  && !$mobile->isTablet()  非平板手机
 * $mobile->is('AndroidOS'); 安卓系统  
 * $mobile->version('iPad');  iPad版本
