...
class UploadFile{
	 ...
	 * @return bool
	 * @is_allow 当is_allow 有值 则不验证 是不是合法上传文件  不用is_upload_file  用于手机端接口 上传图片
	 */
	public function upfile($field,$is_allow){
...
//				}
		...
		if ($this->error != '') return false;//return $this->upload_file;
		if(@move_uploaded_file($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name) || copy($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name)){
			
			//设置权限
			@chmod(BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name,0755);
			//产生缩略图
			if ($ifresize){
...
//		$this->setErrorFileName($this->upload_file['tmp_name']);
	 ...
	 * @param  [type] $is_allow [description]
	 * @return [type]           [description]
	 */
	public function multiple_upfile($field,$is_allow){
		//上传文件
		$this->upload_file = $_FILES[$field];
		
		# 循环检查临时名称是否为空
		/* 去除空数组 */
		foreach ( $this->upload_file['tmp_name'] as $k => $v )
		{
			if ( empty($v) )
			{
				unset( $this->upload_file['tmp_name'][$k] );
			}
		}
		...
		//验证文件大小
		foreach ( $this->upload_file['size'] as $k => $v  )
		{
			if( $v > $this->max_size*1024 ){
				$error = Language::get('upload_file_size_cant_over').$this->max_size.'KB';
				$this->setError($error);
				return false;
			}
		...
		//文件后缀名	先暂时取首位数组
		$tmp_ext = explode(".", $this->upload_file['name'][0]);
		$tmp_ext = $tmp_ext[count($tmp_ext) - 1];
		$this->ext = strtolower($tmp_ext);
        ...
        $this->file_name[$k] = (empty ( $this->fprefix ) ? '' : $this->fprefix . '_') . $tmp_name . '.' . ($this->new_ext == '' ? $this->ext : $this->new_ext);
		...
		if ($this->error != '') return false;//return $this->upload_file;
		if ( is_array($this->upload_file['tmp_name'])  )
		{
			foreach ( $this->upload_file['tmp_name'] as $k => $v )
			{
				if ( @move_uploaded_file( $v, BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name[$k] ) || copy( $v,BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name[$k] ) )
				{
					@chmod(BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name[$k],0755);
				}
			}
			return true;
		}
